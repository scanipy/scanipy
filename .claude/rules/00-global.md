...
The verdict **drives the check's conclusion**: an "Enforce review verdict" step parses the reviewer's final `SCANIPY-REVIEW-VERDICT=…` sentinel (from the action's run transcript) and exits non-zero unless it is `APPROVE`, so a REQUEST-CHANGES — or a missing/uncertain verdict — turns the `claude-review` check **red** (fail-closed). It is no longer merely an advisory comment.

This is the **sole** code-review surface — the former `/code-review-cmp` Skill is retired (one doctrine, one place). Server-side required-status-checks are unavailable on this repo (GitHub Free/private — branch protection and rulesets both 403), so RULE-10 is a **process-level gate** (a loud red check), enforced the same way as the four CI gates and the PR-only-merge shim (`.github/workflows/enforce-pr-only-merges.yml`); wire `claude-review` into `required_status_checks` once the org upgrades to Team/Pro. To re-run the gating check after a fix, **re-fire its trigger — toggle the PR draft↔ready, or close→reopen it**; a `@claude review` comment runs the on-demand `@claude` agent (`.github/workflows/claude.yml`) for fixes/questions/fresh findings but does **not** re-run the `claude-review` job, so it cannot clear the check.
