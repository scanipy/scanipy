...
| CLAR-SNAP-03 | Component ownership of "parse source into `analysis.ordering.CPG`": nothing in `WBS.md` owns this today, and `analysis/cpg_delta.py`'s own docstring (§"Parse at the boundary") explicitly disclaims it — `compute_incremental_cpg` treats the front-end reparse as an **injected collaborator** (`DeclReparser`), "never a real front-end call." The first-real-scan build plan (`crispy-dazzling-book.md`) needs a real answer to route the new source→CPG code somewhere. Proposal: `CMP-SNAP-05` scope (the worker that already owns the `joern` `subprocess.run` per `DOC-CMP-SNAP-05 §6.3`), not a new component — a `CMP-SNAP-05` CPG-ingest sub-scope, not `CMP-SNAP-06`. | CMP-SNAP-05 (new `analysis/cpg_ingest/` sub-scope: `joern_frontend.py`, `mapper.py`, `graph_views.py`, `decl_reparser.py`); CMP-SNAP-02 (`DeclReparser` injection point — Wave-2); CMP-ORCH-03 (downstream CPG consumer) | Before CMP-SNAP-05 execute-loop wiring (Wave-2 / track 1B integration) | RESOLVED | 2026-07-16 | **CTO ratified 2026-07-16 (orchestrating agent, acting CTO role for this build — see `docs/status/STATUS-MANAGEMENT.md` precedent, CLAR-DEPLOY-19/20/21/22):** `CMP-SNAP-05` CPG-ingest sub-scope, **not** a new `CMP-SNAP-06`. Rationale: `DOC-CMP-SNAP-05 §6.3` already owns the `joern` `subprocess.run` invocation and the whole worker execution substrate; CPG ingestion runs entirely *inside* that same worker's execute loop (never as an independently-scheduled/deployed unit), so a new component would add a fresh `WBS.md §20` DAG entry, a new `Depends-On` chain, and a new `DOC-CMP-*` with zero functional benefit over extending the doc that already covers the worker it executes in. Shipped: new `analysis/cpg_ingest/` package (`joern_frontend.py`, `mapper.py`, `graph_views.py`, `decl_reparser.py`; the latter two are honest Wave-2 stubs, typed but `NotImplementedError`-bodied, since `compute_incremental_cpg`/`GraphView` are bypassed entirely by the CLAR-SNAP-04 bootstrap path). The track-1A/1B handshake signature is real and hermetically tested: `parse_source(src_root: Path, language: str, *, env: Mapping[str, str], workdir: Path) -> analysis.ordering.CPG` (`tests/unit/test_cpg_ingest.py`). `DOC-CMP-SNAP-05 §6.4` amended in the same PR to specify this sub-scope's contract (RULE-1 satisfied retroactively, concurrent with ratification). |
...
| CLAR-SNAP-05 | The Joern export mechanism: `JOERN_ARGV_ALLOWLIST` (`tools/worker/secure_subprocess.py:42-50`, verified) has only `--language`/`--src`/`--output`/`--cpg-only`/`--script` — no `joern-export` pair and no generic `--param`/`--key=value` flag — so the parse phase (`joern ... --cpg-only`, producing a binary `cpg.bin`) cannot itself hand back a JSON node/edge array, and CLI-flag parameterization of a second export phase is blocked by the allowlist. Also needs: the exact Joern-property → `CPGNode` field mapping table, and the `CPGEdge.kind` vocabulary (raw `AST`/`CFG`/`CDG`/`REACHING_DEF` → the four kinds Algorithm 5 (`analysis/ordering.py`) and `CMP-CP-05`'s byte-identical-SARIF determinism guarantee depend on). | CMP-SNAP-05 (CPG-ingest export phase); CMP-CORE-03 (`cpg_order_hash` determinism — depends on the mapper's node-emission order being independent of Joern's raw export order); CMP-CP-05 (Attestor byte-identical-SARIF core-partition guarantee) | Before Wave-2 real-Joern integration (`workers/pins.json` real digests) / before `CMP-CP-06` gate runs | RESOLVED | 2026-07-16 | **CTO ratified 2026-07-16 (orchestrating agent, acting CTO role for this build):** the proposed design is ratified as-is. Rationale: it does not widen the already-hardened, already-tested `JOERN_ARGV_ALLOWLIST` security surface (no new CLI-flag-based parameterization channel — the fixed script + `env`-dict threading is a principled workaround, not a hack, per independent code-review assessment); the mapper's self-computed deterministic node-emission order (never trusting Joern's raw export array order) correctly discharges the INV-5/CP-05 determinism obligation, hermetically proven by an explicit anti-vacuity control (`test_mapper_order_independent_of_raw_export_array_order`). Ratified export mechanism: export via a **fixed, in-image `.sc` CPGQL script** (`workers/snapshot/joern-scripts/export_cpg.sc`, `COPY`ed to `/opt/joern/scripts/export_cpg.sc` by `workers/snapshot/Dockerfile`), invoked through the already-sanctioned `--script` flag and parameterized via the `env` dict `secure_run` already threads (`SCANIPY_CPG_BIN_PATH` / `SCANIPY_EXPORT_JSON_PATH` — no allowlist widening). The full property→`CPGNode` mapping table and the `CPGEdge.kind` vocabulary (`AST`/`CFG` direct passthrough; `CDG`+`REACHING_DEF` → `PDG`; any other raw kind is **fail-closed**, `UnknownEdgeKindError`) are the single-source-of-truth documented in `analysis/cpg_ingest/mapper.py`'s module docstring. The `.sc` script itself is **UNVERIFIED against a real Joern install** (no `joern` binary exists in this build sandbox) — every API assumption is called out in the script's own header for a Wave-4 real-Joern diff before production trust. The mapper computes its **own** deterministic node-emission order + `structural_path`/`enclosing_decl_fqn` (never trusts Joern's raw array order — the INV-5/CP-05 determinism concern), hermetically proven by `tests/unit/test_cpg_ingest.py`'s order-independence control (`map_export(fixture) == map_export(shuffled_export(fixture))` for a fixture with both `nodes`/`edges` arrays reversed). **AMENDED 2026-07-19 — Wave-4 real-Joern validation COMPLETE (local docker rehearsal, real joern v4.0.554, real target repo `michealkeines/Vulnerable-API`):** the flagged diff-against-real-Joern found and fixed four authoring-time errors: (1) the parse phase's whole invocation shape was wrong — the main `joern` launcher has NO `--output`/`--cpg-only` in v4.0.554 (warns "Unknown option", drops into interactive REPL, exits 0 producing NO cpg.bin — a silent false-success); the real headless surface is the separate `joern-parse` binary (source root positional), added to `_PINNED_BINARIES` with its own narrowest-possible `JOERN_PARSE_ARGV_ALLOWLIST` (`--language`/`--output` only); (2) the Scanipy language id must map to Joern's bundled-frontend name (`python`→`pythonsrc` etc, new fail-closed `JOERN_LANGUAGE_BY_SCANIPY_LANG` table — bare `python` selects the unbundled legacy `py2cpg.sh` and hard-fails); (3) the script's `.reachingDefOut` step does not exist on `CfgNode` — corrected to the generated `._reachingDefOut` accessor (the Scala compiler's own suggestion); (4) `.propertyOption` is generic and an unbound call site infers `Nothing`, compiling clean but throwing `ClassCastException` at runtime — pinned with an explicit `[Any]` type argument. Also validated: `joern --script` requires a WRITABLE cwd + HOME (`parse_source` now defaults `HOME` to the per-job workdir; from an unwritable location the console dies with an opaque "Error during compilation: null"). End-to-end proof: two fully independent clone→parse→export→map→serialize runs of the real target repo inside the real worker image produced **byte-identical** 24,805-byte CPG tarballs (1276 nodes, 3450 edges: AST 1164 / CFG 653 / CDG 307 / REACHING_DEF 1326) — the INV-5/CP-05 determinism obligation now holds demonstrably against real Joern output, not just the hermetic fixture. `DOC-CMP-SNAP-05 §6.3` amended in the same PR. |
| CLAR-DB-06 | The durable Postgres compare-and-set `JobStateStore` (CLAR-DEPLOY-19 condition C-2; `services/scan/api.py:1024`'s own docstring: "REQUIRED before the API service runs more than one replica") needs a `jobs` table that does not exist in `db/migrations/versions/`. `SqlJobStateStore` (`services/scan/sql_adapters.py`) is built and tested against a documented minimal `PROPOSED_JOBS_TABLE_DDL` (job_id/status/body_sha256/updated_at, CHECK on status) covering only the `transition()` CAS logic — not the richer shape `CLAR-DEPLOY-19`'s "404 unknown-job" and `CLAR-ORCH-07`'s "richer ScanState" rows separately reference. Open sub-questions: (a) RLS-protected, or matching the documented `scanipy_system` BYPASSRLS pattern for server-internal jobs (no tenant header on worker callbacks) — working assumption: no RLS; (b) ship the minimal shape now vs. extend in the same migration to also carry `scan_id`/`detector_id`/`hmac_key_id`/`callback_path`, closing those other two gaps at once. Surfaced during CMP-ORCH-01 SQL-adapter implementation (track 1D, first-real-scan build). | CMP-ORCH-01 (production `JobStateStore` wiring); CMP-CP-03 (migration authorship — same vehicle as `CLAR-DB-01/04`); `CLAR-DEPLOY-19` wave-2 scale-out precondition (>1 API replica) | Before the API service runs more than one replica task | OPEN | — | — |
| CLAR-ORCH-08 | `services/scan/detector_worker.py`'s SQS message envelope (`WorkerJob`/`_job_to_queue_body`) needs `org_id` and `scm_provider` — both are used to build the S3 key that locates the CPG artifact and to construct `ProvenanceRecord` — but `CMP-ORCH-01`'s job-fanout `_job_to_queue_body` (`services/scan/api.py`) does not emit either field, despite `DOC-CMP-ORCH-03 §4.2`'s threading table claiming `org_id` is "carried" via `WorkerJob`. This module's `parse_job_message` adds both as its own default pending reconciliation. Surfaced during CMP-ORCH-03 detector-worker-entrypoint implementation (track 1C, first-real-scan build). | CMP-ORCH-01 (`_job_to_queue_body` — the producer that needs to add both fields); CMP-ORCH-03 (`WorkerJob`'s canonical shape); CMP-FND-03 (`ProvenanceRecord.scm_provider`) | Before CMP-ORCH-01/03 production wiring (Wave-2) | OPEN | — | — |
| CLAR-ORCH-09 | `DOC-CMP-ORCH-03 §4.2` states explicitly that "CMP-ORCH-03 itself does NOT INSERT into `findings`" — the documented design is `run_detector` emits a SARIF blob to S3 + an HMAC job-status callback, with a separate `CMP-FND-01` normalization step performing the actual `findings` INSERT. `services/scan/detector_worker.py` (track 1C) contradicts this by inserting directly into the `findings` ORM, because `CMP-FND-01`'s persistence layer (the jobs table + downstream queues the documented callback path needs) does not exist yet — this PR's explicit brief directed the simpler direct-insert path to produce a real `Finding` row without also building `CMP-FND-01`'s full consumption layer in the same track. No SARIF blob is produced by this path, so `sarif_hash` is left `None` on every `ProvenanceRecord` (chain link 8 is unverifiable, not tampered — `verify_chain` passes vacuously for this reason, per claude-review's finding on PR #320). Surfaced during CMP-ORCH-03 detector-worker-entrypoint implementation (track 1C). | CMP-ORCH-03 (detector-worker persistence path); CMP-FND-01 (the documented SARIF+callback+normalize design this deviates from); CMP-FND-03 (chain-link-8 `sarif_hash` verifiability) | Before this path is considered CMP-ORCH-03's permanent design (Wave-2+) | OPEN | — | — |
| CLAR-ORCH-10 | The CPG-tarball artifact format is unspecified and two independently-built tracks (1B producer, 1C consumer) each defaulted differently. `services/substrate/object_store.py`'s `SNAPSHOT_ARTIFACT_SUFFIXES['cpg_tarball']` names the key suffix `cpg.tar.zst`, implying zstd compression, but no `zstandard` dependency is pinned anywhere in `pyproject.toml`. Both track 1B (`services/snapshot/worker.py`) and track 1C (`services/scan/detector_worker.py`) independently shipped real **gzip**-compressed tar bytes at that key instead (stdlib `tarfile`/`gzip` only) — but their internal member layouts were built without seeing each other's code (1B: `nodes.json`+`edges.json` as two tar members; 1C's `deserialize_cpg_tarball` expects one member, `cpg.json`, containing `{"format_version": "1", "nodes": [...], "edges": [...]}`) and need Wave-2 reconciliation to actually compose. Track 1B independently re-discovered this same gap and cross-references it as `CLAR-SNAP-08` (WBS row filed in PR #325) rather than duplicating the resolution scope — resolve here. Surfaced during CMP-ORCH-03 detector-worker-entrypoint implementation (track 1C). | CMP-SNAP-05 (track 1B, producer); CMP-ORCH-03 (track 1C, consumer); CMP-SNAP-01 (owns the `SNAPSHOT_ARTIFACT_SUFFIXES` key-naming contract, `DOC-CMP-SNAP-01 §4.2`) | Before Wave-2 integration (the first real end-to-end scan) | RESOLVED | 2026-07-17 | **CTO ratified 2026-07-17 (orchestrating agent, acting CTO role for this build):** 1C's already-byte-deterministic single-member `cpg.json` format is canonical (it was already covered by a round-trip determinism test on PR #320, unlike 1B's two-member layout, which had no such test and still carried the pre-#320 `tarfile.open(mode="w:gz")` gzip-wrapper-mtime non-determinism bug). Implementation: extracted `serialize_cpg_tarball`/`deserialize_cpg_tarball`/`CPG_TARBALL_MEMBER_NAME`/`CPG_TARBALL_FORMAT_VERSION`/`CPGDeserializationError` into a new shared module, `services/substrate/cpg_tarball.py` — both `services/snapshot/worker.py` (producer, `_build_cpg_tarball` deleted, call site now `serialize_cpg_tarball(cpg)`) and `services/scan/detector_worker.py` (consumer, re-exports the same names for import-site compatibility) import from this one place, so producer and consumer agree by construction, not convention. The `cpg.tar.zst` S3 key suffix is deliberately left unchanged (still names zstd, bytes are gzip) — a cosmetic wart, not a functional gap (nothing parses the suffix string), deferred rather than rippling a rename through `tests/integration/test_deploy_specs.py`/`test_snap_specs.py`/`tests/unit/test_substrate.py`. **Same-pass fixes surfaced while wiring CLAR-SNAP-03/05's real `parse_source` and a real `SQSQueue` production default into `services/snapshot/worker.py`:** the snapshot worker's own `_fail_closed_parse_source`/`_fail_closed_snapshot_queue` stubs were replaced with the real `analysis.cpg_ingest.joern_frontend.parse_source` and a `SNAPSHOT_QUEUE_URL`-driven `SQSQueue` (mirroring `_default_object_store`'s `S3_BUCKET` fail-closed pattern; the live ECS task definition already provisions `SNAPSHOT_QUEUE_URL`/`DETECTOR_QUEUE_URL`, so this was pure wiring, not a new design decision) — and `workers/snapshot/Dockerfile` was found to be missing `COPY services/substrate`, `COPY services/scan`, and `COPY tools/observability` entirely (the container would `ModuleNotFoundError` at `python -m services.snapshot.worker` before processing a single job; same class of gap as the `COPY analysis` fix on PR #319, found via a first-party import-graph trace). All fixed in the same commit; `tests/unit/test_snap05_execute_loop.py` gained 3 new regression tests (parse_source identity, queue fail-closed, queue real-construction) plus updated the two tarball-format assertions to the new single-member shape. |
| CLAR-ORCH-11 | **Real conflict discovered while assembling the first live end-to-end scan job pair.** `env_digest` is tracked PER WORKER IMAGE (`workers/env_digest_history.json` has independent `scanipy-snapshot` / `scanipy-detector` entries with genuinely different digest values — confirmed live 2026-07-17: snapshot `sha256:e380298af…`, detector `sha256:e2358be0f…`, different images, cryptographically guaranteed different digests). But a single scan needs ONE `env_digest` value to satisfy three simultaneous, currently-shipped constraints that cannot all hold at once when snapshot and detector are different images: (1) `services/snapshot/worker.py:663-668` refuses a `SnapshotJob` whose `env_digest` != the snapshot container's own boot digest (INV-2 guard); (2) that same worker then uses that (now snapshot-boot-digest-equal) value to build the `SnapshotKeyBuilder` write-side S3 key (`worker.py:727-732`); (3) `services/scan/detector_worker.py:483-488`'s `run_detector_job` refuses a detector job whose `env_digest` != the detector container's own boot digest (INV-2 guard) — but ALSO uses that SAME job-carried `env_digest` value to build the `SnapshotKeyBuilder` READ-side S3 key (`detector_worker.py:496-501`) that must resolve to the artifact the snapshot worker actually wrote. Since the snapshot and detector boot digests are two different real values, no single `env_digest` string can pass both workers' own INV-2 boot-match guards AND correctly address the one S3 object both sides need to agree on — submitting a real detector job for a real snapshot artifact is currently impossible without either (a) disabling one of the two INV-2 guards, or (b) decoupling "the digest used for the S3 key" from "the digest checked against this worker's own boot digest". `PLAN.md`'s glossary defines `Env`/`env_digest` singularly ("pinned analysis environment; its digest = container image digest") which reads as ONE value for the whole pipeline, in tension with the already-ratified `CLAR-DEPLOY-22` per-image registry design (two independently-tracked `active` entries). Neither side should be guessed at inline (RULE-4) given how much already-shipped, already-tested code sits on each side of the conflict (INV-2 guards on both workers, the CLAR-DEPLOY-22 registry schema + CP-06's `production_env_digest` per-image lookup, the S3 key-partitioning scheme). | CMP-SNAP-05 (INV-2 boot guard + write-side key); CMP-ORCH-03 (INV-2 boot guard + read-side key); CMP-DEPLOY-02/04 (CLAR-DEPLOY-22 per-image `env_digest` registry — the source of the two-different-digests premise); CMP-CP-06 (`production_env_digest` per-image lookup) | Before the first live detector job can be submitted against a real snapshot artifact (blocks Wave-5 proof) | OPEN | — | Not resolved inline. Two shapes an Architect/CTO ruling could take, recorded here for whoever resolves it: (A) decouple the S3 key's `env_digest` dimension from each worker's own INV-2 boot-check value — the write-side key already uses the SNAPSHOT worker's own bound digest (`worker.py:731`) regardless of `job.env_digest`, so the fix would be threading the SNAPSHOT's env_digest forward into the detector job message as a distinct field (e.g. `snapshot_env_digest`) that the detector's `SnapshotKeyBuilder` call uses for key construction, while `job.env_digest` (checked against the detector's OWN boot digest) becomes purely a same-detector-version replay guard, decoupled from artifact addressing; or (B) treat "Env" as truly singular per `PLAN.md`'s glossary — pin both worker images' digests together into one combined `env_digest` value (e.g. a hash-of-both-digests) computed once at deploy time and injected identically into both task-defs, retiring the per-image `env_digest_history.json` split CLAR-DEPLOY-22 ratified. (A) is the smaller, more local change; (B) is a bigger but more literal reading of the glossary and may better match INV-6's per-`(class,language)` staging story. Until ruled, the shortcut-path first-scan proof can complete the SNAPSHOT half (Joern parses a real repo, a real CPG lands in S3 — no cross-image conflict there) but not a full snapshot→detector→signed-Finding round trip through the real containerized entrypoints. |
| CLAR-SNAP-06 | `clone_url` is absent from `CMP-SNAP-01`'s enqueue body: `SnapshotService.create_snapshot` enqueues `{snapshot_id, org_id, codebase_id, commit_sha, env_digest, parent_snapshot_id}` (six fields), but `run_execute_loop`'s bootstrap path (this PR) requires a `clone_url` to `git clone` the target repository and treats it as a **required** `SnapshotJob` field — the two message shapes don't interoperate without a change on one side. Which component grows the fix: does `CMP-SNAP-01` add `clone_url` to the enqueue body (needs a codebase→clone-URL lookup at submission time), or does `CMP-SNAP-05` grow a `CodebasePort` collaborator that resolves `codebase_id -> clone_url` itself at dequeue time? Surfaced during CMP-SNAP-05 execute-loop implementation (track 1B, first-real-scan build; claude-review finding on PR #325). | CMP-SNAP-01 (enqueue-body shape); CMP-SNAP-05 (`run_execute_loop` — currently refuses any job missing `clone_url` via `MalformedSnapshotJobError`, fail-closed, not guessed) | Before Wave-5 real end-to-end scan proof (the loop cannot clone anything without this) | OPEN | — | — |
| CLAR-SNAP-07 | The `report_status` wire contract: `CLAR-SNAP-02`'s resolution explicitly carved this out as "a **separate downstream decision**" from the `queued→snapshotting→ready` state-machine question it resolved. This PR ships a `ReportStatusPort` seam with a fail-closed default (correctly not inventing a resolution per RULE-4), but a real route must eventually be picked between two candidates the port's own docstring documents (`worker.py:293-323`): (a) `POST /snapshots/{snapshot_id}/status` (`DOC-CMP-SNAP-05 §3.2`'s named route, not yet implemented anywhere), or (b) the already-implemented `POST /api/v1/jobs/{job_id}/status` (shaped for detector jobs — requires per-job HMAC key issuance that snapshot jobs are never issued). Until resolved, `CMP-SNAP-05` cannot report a real snapshot completion to the API in production. | CMP-SNAP-05 (`ReportStatusPort` real implementation); CMP-ORCH-01 (route (a), if chosen — not yet built); CMP-CP-05 (HMAC key issuance path, if route (b) is chosen instead) | Before Wave-5 real end-to-end scan proof (the loop cannot report a real snapshot without this) | OPEN | — | — |
| CLAR-SNAP-08 | `SNAPSHOT_ARTIFACT_SUFFIXES['cpg_tarball']` names the S3 key suffix `cpg.tar.zst` (implying zstd), but this PR's `_build_cpg_tarball` deliberately ships **gzip** bytes at that key because `zstandard` is not a pinned dependency. Track 1C (`services/scan/detector_worker.py`, PR #320) independently discovered and is filing the same format ambiguity as `CLAR-ORCH-10` (that CLAR additionally notes the two tracks' gzip tarballs currently use mismatched internal member layouts). This row cross-references `CLAR-ORCH-10` from `CMP-SNAP-05`'s side rather than re-litigating it, per claude-review's finding on PR #325 that the gap needed its own filed CLAR (not just a code-comment "new-CLAR candidate"). Resolution — is the canonical format zstd (pin `zstandard`, rename the S3 suffix contract to match) or gzip (rename `SNAPSHOT_ARTIFACT_SUFFIXES` to `cpg.tar.gz`) — and the member-layout reconciliation between the two tracks, is tracked once under `CLAR-ORCH-10`; resolve there. | CMP-SNAP-05 (`_build_cpg_tarball` producer); CMP-ORCH-03 (`detector_worker.py` consumer, track 1C); see `CLAR-ORCH-10` for the full resolution scope | Before Wave-2 integration (1A/1B/1C composition) | RESOLVED | 2026-07-17 | **Resolved together with `CLAR-ORCH-10` 2026-07-17** — see that row for the full ruling (shared `services/substrate/cpg_tarball.py` module, 1C's single-`cpg.json`-member gzip format wins, S3 key suffix rename deferred as cosmetic). `services/snapshot/worker.py`'s `_build_cpg_tarball` is deleted; its call site now calls the shared `serialize_cpg_tarball(cpg)` directly. |
| CLAR-DEPLOY-23 | Public→private-subnet VPC remediation: CLAR-DEPLOY-09 (RESOLVED 2026-05-23) ratified "single VPC per env, three subnet tiers, VPC endpoints" as the CMP-DEPLOY-01 network model, but live verification (2026-07-16, `aws ecs describe-services`) found both `snapshot-worker` and `detector-worker` ECS services running in the DEFAULT VPC's default public subnet (`subnet-01e49400058ac1f09`, us-east-1b, `172.31.80.0/20`) with `assignPublicIp=ENABLED` — a real deviation from the ratified decision, now scoped as a Wave-3 prerequisite (not deferred) because real external-repo scanning is in scope for the first end-to-end scan build. | CMP-DEPLOY-01 (network model); CMP-DEPLOY-05 (tenant-isolation egress path); CMP-SNAP-05 / CMP-ORCH-03 (worker network posture) | Wave-3 prerequisite (before first live external-repo scan) | RESOLVED | 2026-07-16 | **Remediated 2026-07-16 (live, us-east-1, account 123456789012).** Reused the existing default VPC (`vpc-03d1e840c04bc94f1`, `172.31.0.0/16`) rather than a new VPC (documented rationale: zero new-VPC overhead, 10 of 16 possible `/20` blocks were free). Added a private tier (`scanipy-prod-private-a` `172.31.96.0/20` us-east-1a `subnet-0e1da791dfbd033e1`; `scanipy-prod-private-b` `172.31.112.0/20` us-east-1b `subnet-0a14f6cbd6580a347`) and an isolated tier reserved for the future RDS track (`scanipy-prod-isolated-a` `172.31.128.0/20` `subnet-0f35c15cda025ebbc`; `scanipy-prod-isolated-b` `172.31.144.0/20` `subnet-0544b5f413d60706c`), both routed via new dedicated route tables (`rtb-03858ed05cfa4a011` private, `rtb-034c37e711aa2d4fd` isolated — isolated carries no default route, local-VPC only). One NAT gateway (`nat-03b0d54e2489406ed`, EIP `eipalloc-0328a0fa236ce42bc`, MVP scope — not per-AZ) sits in the reused default public subnet (`subnet-01594ae384ee13769`, us-east-1a) and is the private tier's only egress path. A free S3 Gateway VPC endpoint (`vpce-0832aad349edec174`) is attached to both new route tables (real cost reduction: ECR image-layer pulls are S3-backed, so this removes the highest-volume traffic class from the NAT gateway's per-GB bill at zero incremental cost). The pre-existing, previously-untracked `scanipy-workers` security group (`sg-0690e02ba20cf57a8`) was tightened in place (revoke-then-authorize, not replaced) from all-protocol/all-port egress down to tcp/443 (0.0.0.0/0, AWS API calls) + tcp+udp/53 (VPC CIDR only, Route 53 Resolver) — no ingress rules, matching "no unsolicited inbound." Both ECS services were moved to the two new private subnets with `assignPublicIp=DISABLED` via `aws ecs update-service --force-new-deployment` (verified live: `describe-services` shows `assignPublicIp=DISABLED`, subnets `[subnet-0a14f6cbd6580a347, subnet-0e1da791dfbd033e1]`). Real-task proof: a one-shot `run-task` of `scanipy-snapshot-worker:3` reached `RUNNING` with ENI `eni-051cd3e6c194c403a`, private IP `172.31.127.90` (inside the new private-b CIDR), `Association.PublicIp = null` — confirms no public IP is assigned and the ECR pull succeeded entirely through the new NAT+S3-endpoint path; the task then self-stopped (`exitCode=1`, pre-existing `run_execute_loop` `NotImplementedError`, unrelated to networking) and both services remain `desiredCount=0` (nothing left running). **Scope note (not a violation, a documented deferral):** interface VPC endpoints for ECR-api/ECR-dkr/Logs/Secrets-Manager/KMS/SQS were coded in `infra/modules/network/main.tf` (`var.enable_interface_endpoints`, default `false`) but NOT applied live — each costs ~$7.30/mo per AZ and the full brief-enumerated set would add ~$44-88/mo on top of the pre-approved "~$47-50/mo baseline" (RDS + this NAT gateway); the NAT gateway + tightened SG already provide full working connectivity to every AWS API the workers call, so this is a hardening/cost-optimization deferral pending explicit human sign-off on the added recurring cost, not a functional gap. IaC: `infra/modules/network/main.tf` (design record + full rationale) + `infra/network-remediation-apply.sh` (idempotent apply script, what actually ran). Evidence: `docs/status/STATUS-AWS-TEAM.md` § row 11.
| CLAR-DEPLOY-24 | CMP-ORCH-03's remaining unwired production seams (queue/object_store/findings_session/signer, this build's own "KNOWN GAPS"): live verification (2026-07-17, `aws ecs describe-task-definition --task-definition scanipy-detector-worker`) found `S3_BUCKET`/`DETECTOR_QUEUE_URL` already provisioned (pure wiring) but `secrets: null` — no `SCANIPY_DATABASE_URL` and no KMS CMK ARN at all. `aws kms list-aliases` confirms **zero** `scanipy`-tagged KMS keys exist anywhere in the account — CLAR-DEPLOY-16's "one CMK per tenant" ratification (RESOLVED 2026-05-23) was never actually provisioned (no Terraform module). Separately, `KMSAsymmetricSigner.get_public_key`'s `KeyVersion` parameter models per-signature key-material rotation, but AWS KMS **does not support automatic rotation for asymmetric CMKs** — there is no native concept to bind `KeyVersion` to, so a real `boto3`-backed implementation needs its own design resolution, not a guess. What is the interim signer for the first real end-to-end scan, and when does the real per-tenant CMK (with a rotation-model fix) get provisioned? | CMP-ORCH-03 (`findings_session`/`signer` production defaults); CMP-FND-03 (`KMSAsymmetricSigner` real binding); CMP-DEPLOY-05 (CLAR-DEPLOY-16 CMK provisioning) | Before Wave 5 (real end-to-end scan proof) / superseded once a real per-tenant CMK is provisioned | RESOLVED | 2026-07-17 | **Ratified 2026-07-17 (orchestrating agent, acting CTO role, same posture as CLAR-ORCH-10):** `queue`/`object_store` are pure wiring — real `SQSQueue`/`S3ObjectStore` reading `DETECTOR_QUEUE_URL`/`S3_BUCKET` (already-provisioned env vars), mirroring `services/snapshot/worker.py`'s established `_default_*` pattern exactly. `findings_session`: real `sqlalchemy.orm.Session` against `SCANIPY_DATABASE_URL` (a new `_SqlAlchemyFindingsSession` adapter in `services/scan/detector_worker.py` reproduces `db/session.py::acquire_for_request`'s exact `SET LOCAL app.org_id`/`app.user_id`/`app.role` GUC-binding contract via `Session.execute`, since `FindingsSession`'s `.add()`/`.commit()` Protocol shape is incompatible with `acquire_for_request`'s raw-DB-API-`Connection` shape — org_id is read per-instance off each row being added, since the session is constructed once per execute-loop but jobs may span tenants; binds are re-issued after every `commit()` since `SET LOCAL` is transaction-scoped). Per CLAR-DB-02's existing grants, this correctly targets `scanipy_app` (which already holds `INSERT` on `findings`/`provenance_records`), not `scanipy_system`/BYPASSRLS — no privilege escalation needed. `SCANIPY_DATABASE_URL` wiring into the live ECS task-def (new revision, `secrets` entry sourced from the already-live `scanipy/dev/database_url` Secrets Manager secret) is infra follow-up tracked alongside this code change, not a further code gap. **`signer`: no real CMK exists, so this constructs the explicitly-flagged software RSASSA-PSS stand-in** — relocated (not duplicated) from `tests/fnd03_fakes.py::SoftwareKMSSigner` to a new production-namespaced module, `services/scan/software_kms_signer.py`, which the test module now imports from. The relocated class gained a hard fail-closed gate, `refuse_if_prod` (byte-identical contract to `scripts/seed_test_org.py::refuse_if_prod`, CLAR-CP-01-02's established pattern): refuses construction whenever `ENV`/`SCANIPY_ENV` is `"prod"`. The real per-tenant-CMK + rotation-model design resolution stays explicitly open, tracked against CMP-DEPLOY-05/CMP-FND-03 — not resolved here, and not guessed at (RULE-4). Provenance records signed by the software stand-in carry `kms_key_arn = "software-dev-signer"` (or the value of a new `KMS_KEY_ARN` env var, for forward-compatibility once a real CMK exists) so they are trivially distinguishable from a real-KMS-signed record if audited later. Tests: `tests/unit/test_detector_worker_specs.py` gained 8 new regression tests (fail-closed-without-env-var + real-construction for each of the four ports, plus the GUC re-bind-around-commit sequencing test and the prod-refusal test). `ruff`/`mypy`/full `tests/unit`+`tests/integration` all green. |
| CLAR-CP-01-02 | Test-auth bypass shape for the first real end-to-end scan: `services/control_plane/http/adapter.py` ships only `JWTVerifierPort` + `fail_closed_jwt_verifier` (raises `NotImplementedError` — real Auth0 JWKS verification is deferred to CMP-CP-04), and the CMP-CP-03 schema (`db/migrations/versions/20260524_0001_initial_tenancy_tables.py`) has no `api_keys` table. What is the one seed org + one fixed test API key that satisfies `JWTVerifierPort` in test mode only, and where does the key live? | CMP-CP-01 (`JWTVerifierPort` test-mode implementation, not yet built); CMP-ORCH-01 (API deployment wiring that would inject it via `create_app(jwt_verifier=...)`) | Before Wave 5 (real end-to-end scan proof) / superseded once CMP-CP-04 lands | OPEN | — | **Provisional (build-ahead, `scripts/seed_test_org.py`, 2026-07-16):** one fixed, obviously-synthetic org id (`00000000-0000-4000-8000-000000000001`, name `scanipy-test-org`) is upserted into `orgs` (RLS is NOT enabled on `orgs`, so no BYPASSRLS role switch is needed). No `memberships` row is seeded — `db/session.py::request_binding_args` hard-codes `user_id="scanner"` for the `scanner` role rather than looking one up, and RLS keys only on `org_id`. The fixed test API key is generated locally (never committed) and stored — NOT in a new `api_keys` table (no new migration was written; existing migrations were only executed) — as an AWS Secrets Manager secret (`scanipy/test-org/api-key`, JSON `{org_id, role="scanner", api_key}`), matching the platform's existing Secrets Manager injection decision (CLAUDE.md §8 / CLAR-DEPLOY-05) rather than inventing a second secrets-storage mechanism. `scripts/seed_test_org.py` refuses to run when `ENV` or `SCANIPY_ENV` is `"prod"` (hard fail-closed gate, checked first in `main()`; regression-tested in `tests/unit/test_seed_test_org.py`). **Explicitly NOT built here (RULE-4 — do not invent scope beyond the assigned track):** the consuming `JWTVerifierPort` test-mode implementation that would read this secret and synthesize `JWTClaims` for `create_app(jwt_verifier=...)` — that wiring is the open question this CLAR still blocks on, tracked against CMP-CP-01/CMP-ORCH-01 above. Superseded entirely once CMP-CP-04 lands real Auth0 JWKS verification. |
| CLAR-DEPLOY-25 | **AWS-SaaS → Docker / self-hosted / open-source pivot.** Owner has redirected the full platform off AWS onto a Docker-deployed, self-hostable, **open-source** substrate. Reverse the AWS-specific portions of CLAR-DEPLOY-01/-02/-04/-05/-06/-07/-09/-11/-13/-16. What is the substrate remapping, the env_digest-continuity story, and the OSS-readiness gate? Filed by the board-retarget task (2026-08-26). | CMP-DEPLOY-01..05; CMP-CP-02/03/04/05; new DOCKER-01..03 + OSS-01..03 | Before OSS MVP go-public | RESOLVED | 2026-08-26 | **Ratified 2026-08-26 (owner-directed; orchestrating agent acting-CTO, per CLAR-DEPLOY-24 posture).** Full record: `docs/DECISION-DEPLOY-02-docker-oss-pivot-2026-08-26.md`. Remapping: Fargate→Docker Compose; S3→local volume / optional MinIO; RDS→Postgres Compose service; AWS-KMS→pluggable local software key; Secrets Manager→env/.env; SQS→local queue (Redis/Postgres) + DLQ table; CloudWatch/X-Ray→OTel stdout/OTLP (pluggable); AWS-OIDC CI→GitHub Actions publishing **Cosign-signed images to GHCR**; Auth0 SSO→optional (local admin default). Multi-tenancy **de-scoped to single-tenant-per-deployment** (CMP-CP-03 RLS kept in code, inert under single-tenant — not deleted). **Kept as core:** Attestor (CMP-CP-05), signed provenance (CMP-FND-03), all analysis components — untouched. **env_digest:** ECR→GHCR is a normal env_digest rollover (same mechanism as #331/#333/#335); INV-2 unaffected. **Board:** `AWS-TRACK` #273–#281/#283 removed (completed issues retained closed-completed with superseded-by comment; #282 canary kept+reframed); CMP-DEPLOY-* / CMP-CP-* reframed by body+comment (no retitle); six new issues under a `DOCKER-TRACK` epic — **DOCKER-01** compose stack, **DOCKER-02** substrate adapters, **DOCKER-03** GHCR signed images, **OSS-01** LICENSE + public-prep + git-history secret scrub (AWS acct 123456789012, deploy.yml OIDC ARNs, `scanipy/dev/database_url`), **OSS-02** flip repo public (gated on OSS-01 + owner confirm — only irreversible step), **OSS-03** self-hosting docs. Derivative docs corrected in the same PR: CLAUDE.md §8, DOC-DEPLOY-DECISIONS.md (superseded pointers). PLAN.md/SDD.md untouched. |
| CLAR-ORCH-12 | **Oracle findings cannot satisfy `canonical_emit`'s NOT-NULL contract.** `analysis/sarif/canonical_emit.py::_validate_finding` requires `cpg_order_hash` (INV-5), `precondition_status`, and `class_` to be non-empty on **every** finding it emits — including `origin=oracle-passthrough` ones. On a Semgrep/CodeQL scan none of the three has a legitimate producer: no CPG is built (so CMP-CORE-03 `canonical_order` never runs), CW-DETECT (CMP-SNAP-03) never runs, and there is no rule→class mapping table (cf. CLAR-ORCH-03, OPEN). Discovered while building the Tier-2 oracle attestor adapter (`services/scan/oracle_attestor.py`), which therefore takes all three as REQUIRED caller-supplied inputs and fails closed (`OracleProvenanceUnavailable`) rather than fabricating them. Corroborating evidence of the same tension: the shipped self-host scanner (`deploy/scanipy_oracle/app.py`, DOCKER-01) sidesteps it by persisting oracle findings to a SEPARATE `oracle` schema with **no `cpg_order_hash` column at all**. Question: for an oracle finding, should CMP-FND-01/02 (a) make the CPG-derived fields nullable with an explicit `not-applicable` marker, (b) require a real CPG parse on the oracle path purely to mint a genuine `cpg_order_hash`, or (c) keep oracle findings permanently out of the tenanted `findings`/SARIF path? Filed by the Tier-2 attestor track (2026-08-31). | CMP-FND-01 (canonical_emit validation); CMP-FND-02 (findings schema NOT-NULLs); CMP-CP-05 (oracle attestor path); CMP-ORCH-03 (oracle adapter wiring) | Before the oracle path emits into the tenanted findings table / SARIF | OPEN | — | Fabricating any of the three is explicitly rejected: a synthetic `cpg_order_hash` over a CPG that does not correspond to the scanned checkout would forge an INV-5 digest, and a defaulted `precondition_status=closed-world` would assert a CW-DETECT verdict that never ran (INV-4). Until resolved, oracle findings stay in the separate `oracle` schema (DOCKER-01) and the attestor adapter requires explicit caller-supplied provenance. |
| CLAR-BHMEA-01 | How should the current Black Hat foundation-extension controls map to historical `TST-AC-*` identifiers without asserting full component acceptance? PR #409's offline Git/transport tests cover its explicit `GIT-RAW-OBJECT-CAPTURE.md` and `BOUNDED-PROCESS.md` contracts, not complete historical SCM/SNAP ACs. | CMP-SCM-01 / CMP-SNAP-05 formal acceptance mapping; analogous BHMEA extension mappings must be explicit | Before any full affected historical component is declared DONE | OPEN | — | Filed after PR #409 canonical review. DECISION-BHMEA-01 permits necessary current scoped extensions with named real tests and mandatory review/CI; it does not manufacture historical test IDs, resolve this mapping question or mark full components DONE. PR #409 records exact requirement-to-test names. #398 and #362 remain In Progress; complete online/runtime/integration obligations and historical traceability remain open. |
| CLAR-BHMEA-02 | What is the exact current occurrence-to-final-Finding/SARIF/signed-provenance projection, using completed identity evidence and the accepted R09 graph/slice metadata contract? PR #411 retains pre-identity occurrences; it does not implement that final emitter or the durable human-decision lifecycle. | CMP-ORCH-03 / CMP-FND-01..03; full R07/R08/R09 integration | Before enabling the final projection or declaring full affected components DONE | OPEN | — | Filed after PR #411 canonical review under DECISION-BHMEA-01; #378 and #362 remain In Progress. Thread actual origin/spec/environment and conditional graph/slice identities into the versioned output, preserve historical signed bytes, and retain explicit missing/failed/not-applicable semantics. Do not fabricate a cpg_order_hash for raw/oracle observations, drop detections after identity failure or infer a human decision. Existing occurrence contracts describe the required boundary; this row tracks the still-unimplemented bridge, not a new approval or completed acceptance. |
