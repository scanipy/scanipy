...
| CLAR-DB-03 | `provenance_records.repartition_oracle_id` and `CMP-SNAP-04` reference a `snap_oracle_runs` table (one row per async differential-oracle run, with both verdicts + `agreed` flag) that `DOC-DB §4` does not yet define. Surfaced by the CLAR-FND-01 reconciliation. | CMP-SNAP-04; CMP-FND-03 FK | Before CMP-SNAP-04 migration ships | OPEN | — | Add `snap_oracle_runs` to DOC-DB §4 (`id` PK per schema convention; align SNAP-04/PROVENANCE `run_id`→`id`). Architect to ratify. |
