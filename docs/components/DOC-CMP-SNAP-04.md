...
2. **Zero or more re-partition records** in `provenance_records` (per `DOC-PROVENANCE §4`) — one per affected finding, each with `record_kind = 'repartition'`, `parent_record_id` pointing to the original `finding`-kind record, and `repartition_oracle_id` referencing the `snap_oracle_runs.run_id`.
...
SELECT
    gen_random_uuid(), affected.record_id, 'repartition', now(),
...
1. **Append-only** — original `record_kind='finding'` records are never UPDATEd; only `record_kind='repartition'` is inserted.
...
| New `provenance_records` rows | provenance table; `record_kind = 'repartition'` | One per affected finding; per `DOC-PROVENANCE §4`. |
...
| `repartition_oracle_id` | new row | FK to `snap_oracle_runs.run_id`. |
...
| **AC-SNAP-04c** | > Every re-partition event is written to provenance. | `TST-AC-SNAP-04c` `[FORTHCOMING]` — assert every re-partition produces a `record_kind='repartition'` row linked to the parent via `parent_record_id` and to the oracle run via `repartition_oracle_id`. |
