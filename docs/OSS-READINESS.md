...
- **No hard credentials in history.** All Postgres DSN matches are env-var references
  ...
  found.
- **Low-sensitivity identifiers present:** the AWS account id `508703380027` (~34 occurrences, from the
  now-superseded AWS track) and one role ARN `arn:aws:iam::508703380027:role/scanipy-ecs-worker`. These
