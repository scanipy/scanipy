...
> **Numbering note:** internal references to “CLAR-DEPLOY-21” inside this record's decision and implementation-contract text predate the register's final numbering and denote **this** record (CLAR-DEPLOY-22). The moto-adoption decision (previous section) is the actual CLAR-DEPLOY-21.
...
- `docs/cross-cutting/DOC-DEPLOY-DECISIONS.md`: add static section `## env_digest history` — one paragraph pointing to `workers/env_digest_history.json` + CLAR-DEPLOY-21 (a pointer, not a mechanically-updated ledger — reconciles DEPLOY-02 §6.1.6 with DEPLOY-04 §6.2.7).
- `docs/status/STATUS-AWS-TEAM.md` row 5: append "**Superseded by CLAR-DEPLOY-21** — registry is `workers/env_digest_history.json`; the v0.1.0 prose nomination is `void`."
