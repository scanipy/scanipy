...
| R16 raw Git producer and process transport | Corpus agent; [#398](https://github.com/scanipy/scanipy/issues/398) | Shared transport reviewed/committed; offline raw Git producer implemented, reviewed and undergoing final primitive-boundary hardening | Finish final tests/hooks and obtain exact-head CI/review. Online Git still needs an approved profile and enforceable egress/runtime controller; offline consistency is not SCM-origin authentication |
...
Merged snapshot base: main `02933f1a08587a7f9f266d47320f5d614ade57ba`
at 14:06 UTC on September 25; local implementation notes include subsequent
...
| [#397](https://github.com/scanipy/scanipy/issues/397), source-bound scalar semantics | Bound-rule/model decoder checkpoint `da0d359` and actual transport integration `8cc62d3`; new stdlib syntax worker has 43 passing independent root tests and scoped review; combined worker run 2,009 passed/51 skipped | Complete pure syntax codec, actual loader/runtime and semantic projection/solver. No accepted witness, operational model or fresh native run; full Java/Python/corpus obligations remain |
| [#398](https://github.com/scanipy/scanipy/issues/398), raw Git acquisition | Reviewed shared transport committed `62a6158`; seven-file offline producer implemented with 158 passing independent root tests after JSON/retention/cleanup fixes; further primitive snapshots in progress | Final tests/hooks remain pending after hardening. One broader run hit an existing canonical wall deadline (1,726 passed/51 skipped/one failure); preserve and investigate. No native Git, image build, online egress controller or SCM-origin authentication |
| [#399](https://github.com/scanipy/scanipy/issues/399), accepted-byte authority verifier | Exact protocols and actual shared dependencies; test-key crypto/diagnostic worker implementation with bounded private runtime copies and hostile-input controls, under final review/tests | Public operational verifier refuses without real loader/controller. Actual operator/root/admission installation, registry publication, durable authority and launcher/API integration remain open |
...
#382's at 14:04 UTC, both with final APPROVE. Those PRs are now merged. Continue
with #389 and remaining candidates serially;
